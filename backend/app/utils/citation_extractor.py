...
def add_citations(response) -> str:
    ...
    sorted_supports = sorted(
        supports, key=lambda s: s.segment.end_index if hasattr(s.segment, "end_index") else 0, reverse=True
    )
    ...
    for support in sorted_supports:
        ...
        if support.grounding_chunk_indices:
            ...
            for chunk_idx in support.grounding_chunk_indices:
                if chunk_idx < len(chunks) and hasattr(chunks[chunk_idx], "web") and hasattr(chunks[chunk_idx].web, "uri"):
