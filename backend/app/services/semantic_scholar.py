...
class SemanticScholarService:
  ...
  def _get_identifier(
    self, doi: Optional[str] = None, arxiv: Optional[str] = None
  ) -> Optional[str]:
    if doi:
  ...
  async def search_paper(self, title: str) -> Optional[str]:
    """Search for a paper by title and return its Semantic Scholar ID or identifier."""
    if not title:
      return None
  ...
  async def get_recommendations(
    ...
    try:
      ...
      print(f"Error fetching recommendations for {identifier}: {e}")
      return []
