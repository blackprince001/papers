...
def upgrade() -> None:
  ...
  op.add_column(
    "discovery_sessions", sa.Column("query_understanding", sa.JSON(), nullable=True)
  )
  ...
  op.add_column(
    "discovery_sessions", sa.Column("relevance_explanations", sa.JSON(), nullable=True)
  )
  # Store papers as JSON for quick loading
  op.add_column(
    "discovery_sessions", sa.Column("papers_json", sa.JSON(), nullable=True)
  )
