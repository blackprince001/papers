...
def upgrade() -> None:
  ...
  op.add_column(
    "papers",
    sa.Column(
      "processing_status", processingstatus, server_default="pending", nullable=False
    ),
  )
  ...
  op.create_index(
    op.f("ix_papers_processing_status"), "papers", ["processing_status"], unique=False
  )
  op.create_index(
    op.f("ix_papers_reading_status"), "papers", ["reading_status"], unique=False
  )
  ...
  op.create_foreign_key(
    "papers_merged_from_paper_id_fkey",
    "papers",
    "papers",
...
def downgrade() -> None:
  ...
  op.create_foreign_key(
    "papers_is_duplicate_of_fkey", "papers", "papers", ["is_duplicate_of"], ["id"]
  )
